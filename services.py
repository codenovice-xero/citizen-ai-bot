...
class StarCitizenService:
    def __init__(self, client: UEXClient, wiki: WikiClient | None = None) -> None:
        self.client = client
        ...
        self._wiki_vehicle_cache: tuple[float, list[dict[str, Any]]] | None = None
        self._wiki_vehicle_cache_ttl: int = 60 * 60 * 6
    ...
    async def _get_wiki_vehicles(self) -> list[dict[str, Any]]:
        now = time.time()
        if self._wiki_vehicle_cache and (now - self._wiki_vehicle_cache[0]) < self._wiki_vehicle_cache_ttl:
            return self._wiki_vehicle_cache[1]

        payload = await self.wiki.get("/vehicles")
        records = self._extract_records(payload)
        self._wiki_vehicle_cache = (now, records)
        return records

    async def search_vehicle(self, ship_name: str) -> dict[str, Any] | None:
        records = await self._get_wiki_vehicles()
        ...
        for record in records:
            ...
            score = fuzzy_score(needle, name.lower())
            contains = needle in name.lower()
            effective = 1.0 if contains else score
    ...
    async def get_ship_loadout_from_wiki(self, ship_name: str) -> dict[str, Any] | None:
        vehicle = await self.search_vehicle(ship_name)
        if not vehicle:
            return None

        identifier = vehicle.get("uuid") or vehicle.get("id") or vehicle.get("slug")
        if not identifier:
            return None

        detail = await self.wiki.get(f"/vehicles/{identifier}")
    ...
    def normalize_ship_loadout(self, payload: dict[str, Any]) -> dict[str, Any]:
        detail = payload.get("detail", {})
        vehicle = payload.get("vehicle", {})
        name = detail.get("name") or vehicle.get("name") or vehicle.get("title") or "Unknown Ship"

        hardpoints = detail.get("hardpoints") or []
        components = detail.get("components") or detail.get("vehicle_components") or []
        ...
        coolers: list[str] = []
        quantum_drives: list[str] = []

        def make_line(obj: dict[str, Any], fallback: str) -> str:
            obj_name = str(
                obj.get("name")
                or obj.get("item_name")
                or obj.get("component_name")
                or obj.get("weapon_name")
                or obj.get("title")
                or fallback
            )
            bits = [obj_name]

            size = obj.get("size") or obj.get("item_size") or obj.get("component_size")
            class_name = obj.get("class_name") or obj.get("class") or obj.get("classification")
            ...
            if class_name:
                bits.append(str(class_name))
            if manufacturer_name:
                bits.append(str(manufacturer_name))

            return " • ".join(bits)

        for hp in hardpoints:
            hp_type = str(hp.get("type") or hp.get("sub_type") or "").lower()
            hp_name = str(hp.get("name") or hp.get("title") or hp_type or "Hardpoint")
            line = make_line(hp, hp_name)

            mounted = hp.get("item") or hp.get("component") or hp.get("weapon") or hp.get("equipped")
            if isinstance(mounted, dict):
                line = make_line(mounted, hp_name)

            if any(word in hp_type for word in ["weapon", "gun", "mount"]):
                weapons.append(line)
            elif any(word in hp_type for word in ["turret", "gimbal"]):
                turrets.append(line)
            elif any(word in hp_type for word in ["missile", "rack", "launcher"]):
                missiles.append(line)

        for comp in components:
            comp_type = str(comp.get("type") or comp.get("sub_type") or comp.get("category") or "").lower()
            line = make_line(comp, "Component")

            if "shield" in comp_type:
                shields.append(line)
            elif "power" in comp_type:
                power_plants.append(line)
            elif "cooler" in comp_type:
                coolers.append(line)
            elif "quantum" in comp_type:
                quantum_drives.append(line)
            elif "missile" in comp_type and line not in missiles:
                missiles.append(line)
            elif ("weapon" in comp_type or "gun" in comp_type) and line not in weapons:
                weapons.append(line)
