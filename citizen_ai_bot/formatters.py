...
def help_embed() -> discord.Embed:
    embed = _base_embed(
        "🤖 Citizen AI Commands",
        "Use slash commands to search items, scan routes, and get Star Citizen guidance.",
...
def format_item_result(matches: list[ItemMatch], locations: list[ItemLocation]) -> discord.Embed:
    ...
    embed = _base_embed(f"📦 {best.name}", " • ".join(subtitle_bits) if subtitle_bits else None, color=discord.Color.blue())

    if best.uuid is not None:
        embed.add_field(name="Item UUID", value=str(best.uuid), inline=True)
...
def format_route(route: RouteSuggestion) -> discord.Embed:
    ...
    }.get(getattr(route, "risk_label", "") or "", discord.Color.green())
    ...
    embed.add_field(name="Risk", value=getattr(route, "risk_label", None) or "n/a", inline=True)
...
def format_route_list(routes: list[RouteSuggestion]) -> discord.Embed:
    ...
    for idx, route in enumerate(routes[:5], start=1):
        value = (
            ...
            f"**Margin:** {fmt_credits(route.margin_per_unit)} • **Est:** {fmt_credits(route.estimated_profit)} • "
            f"**Risk:** {getattr(route, 'risk_label', None) or 'n/a'}"
        )
...
def format_loadout(loadout: LoadoutReport | None, requested_ship: str) -> discord.Embed:
    if not loadout:
        return _base_embed(
            f"🛠️ {requested_ship}",
            "No ship data was returned by the Star Citizen Wiki API for that query.",
            color=discord.Color.red(),
        )
...
def format_mining(plan: MiningSuggestion | None, requested_ship: str) -> discord.Embed:
    if not plan:
        return _base_embed(f"⛏️ {requested_ship}", "No curated mining entry yet.", color=discord.Color.red())
