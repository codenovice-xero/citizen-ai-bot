...
def format_item_result(matches: list[ItemMatch], locations: list[ItemLocation]) -> discord.Embed:
    if not matches:
        return _base_embed("🔎 Item Search", "No item matches found. Try a broader name or a shorter keyword.", color=discord.Color.red())
...
def format_route_list(routes: list[RouteSuggestion]) -> discord.Embed:
    ...
    for idx, route in enumerate(routes[:5], start=1):
        value = (
            ...
            f"**Margin:** {fmt_credits(route.margin_per_unit)} • **Est:** {fmt_credits(route.estimated_profit)} • **Risk:** {getattr(route, 'risk_label', None) or 'n/a'}"
        )
